...
def render():
    ...
    with tab_intake:
        ...
        with st.form("intake_form", clear_on_submit=True):
            ...
            intake_date  = c1.date_input("입고일 *", value=date.today())
            intake_type  = c2.selectbox("입고분류 *", type_names)
            expected_out = c3.date_input("출고예정일", value=None)

            c4, c5, c6 = st.columns(3)
            plate_number = c4.text_input("차량번호 *", placeholder="12가3456")
            model        = c5.text_input("차량모델", placeholder="BMW 520d")
            color        = c6.text_input("차량색상", placeholder="흰색")

            c7, c8 = st.columns(2)
            vin     = c7.text_input("차대번호(VIN)", placeholder="선택입력")
            mileage = c8.number_input("입고 주행거리(km)", min_value=0, value=0, step=1000)

            memo = st.text_area("비고", height=80)

            st.subheader("② 고객 정보")
            cust_mode = st.radio("고객", ["신규 고객 등록", "기존 고객 검색"], horizontal=True)

            customer_id = None
            if cust_mode == "신규 고객 등록":
                cc1, cc2 = st.columns(2)
                cust_name  = cc1.text_input("고객명 *")
                cust_phone = cc2.text_input("연락처", placeholder="010-0000-0000")
                cust_memo  = st.text_input("고객 메모")
            else:
                ...
                sel  = st.selectbox("고객 선택", ["-- 선택 --"] + list(opts.keys()))
                if sel != "-- 선택 --":
            ...
            if is_insurance:
                ...
                vehicle_type  = bi2.selectbox("차량구분", ["국산", "외산"])
                deductible    = bi3.number_input("자기부담금(원)", min_value=0, step=10000)
                fault_ratio   = bi4.number_input("과실비율(%)", min_value=0.0, max_value=100.0, step=5.0)
                vat_applicable = st.checkbox("부가세 적용", value=True)
            else:
                insurance_co = vehicle_type = None
                deductible = fault_ratio = 0
                vat_applicable = False

            submitted = st.form_submit_button("✅ 입고 등록", type="primary", use_container_width=True)
